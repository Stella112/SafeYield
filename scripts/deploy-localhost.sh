#!/usr/bin/env bash
# Deploy SafeYield to a running anvil node at 127.0.0.1:8545 and regenerate
...
if ! PRIVATE_KEY="$ANVIL_PK" forge script script/DeploySafeYield.s.sol:DeploySafeYield \
    --rpc-url "$RPC_URL" \
...
grep -E "SafeYield|Owner|===" "$deploy_log" || true
...
echo "    packages/nextjs/contracts/SafeYield.ts (+ SafeYield.local.ts)."
